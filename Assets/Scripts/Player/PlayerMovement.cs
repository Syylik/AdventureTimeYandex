...
public class PlayerMovement : Movement {
    ...
    public void SetJump(bool isPressed, bool wasReleased)
    {
        didJump = canHoldJump ? isPressed: wasReleased;
    }
    ...
    public void Move(Vector2 moveAxis)
    {
        ...
        {
            RaycastHit[] hits = Physics.SphereCastAll(transform.position - new Vector3(0, ((capsule.height / 2) * transform.localScale.y) - 0.01f, 0), capsule.radius, Vector3.down, 0, Physics.AllLayers);
    ...
    public void Look(Vector2 lookAxis)
    {
        if(enableCameraMovement)
        {
            ...
            mouseYInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.X ? lookAxis.y : -lookAxis.y;
            mouseXInput = mouseInputInversion == InvertMouseInput.None || mouseInputInversion == InvertMouseInput.Y ? lookAxis.x : -lookAxis.x;
...
#if UNITY_EDITOR
    [CustomEditor(typeof(PlayerMovement)),InitializeOnLoadAttribute]
    public class FPAIO_Editor : Editor{
        ...
        #region Camera Setup
            ...
            t.mouseInputInversion = (PlayerMovement.InvertMouseInput)EditorGUILayout.EnumPopup(new GUIContent("Mouse Input Inversion","Determines if mouse input should be inverted, and along which axes"),t.mouseInputInversion);
        ...
        #region Movement Setup
            ...
            t.moveSpeed = EditorGUILayout.Slider(new GUIContent("Walk Speed","Determines how fast the player walks."),t.moveSpeed,0.1f,10);
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            t.fsmode = (PlayerMovement.FSMode)EditorGUILayout.EnumPopup(new GUIContent("Footstep Mode","Determines the method used to trigger footsetps."),t. fsmode);
            EditorGUILayout.Space();

            #region FS Static
            if(t.fsmode == PlayerMovement.FSMode.Static){
                ...
                t.dynamicFootstep.materialMode = (PlayerMovement.DynamicFootStep.matMode)EditorGUILayout.EnumPopup(new GUIContent("Material Type", "Determines the type of material will trigger footstep audio."),t.dynamicFootstep.materialMode);
                EditorGUILayout.Space();
                #region Wood Section
                ...
                if(showWoodFS){
                    GUILayout.BeginVertical("box");
                    if(t.dynamicFootstep.materialMode == PlayerMovement.DynamicFootStep.matMode.physicMaterial){
           ...
           /* EditorGUILayout.BeginVertical("Box");
            ...
            EditorGUILayout.SelectableLabel(new GUIContent("Здесь была реклама").text,new GUIStyle(GUI.skin.label){alignment = TextAnchor.MiddleCenter,fontStyle = FontStyle.Bold, fontSize = 13},GUILayout.ExpandWidth(true));
